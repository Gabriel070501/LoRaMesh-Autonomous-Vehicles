...
densities = vehicle_counts / Z; % densidade: veículos por km²
...
% Plotar densidade vs taxa de colisão
figure;
plot(densities, collision_rates, 'LineWidth', 2, 'Color', [0 0.5 0]);
grid on;
xlabel('Densidade de veículos (nós / km²)');
ylabel('Taxa de colisão (fração de bits perdidos)');
title('Probabilidade de colisão em função da densidade de veículos');
ylim([0 1]);
